...
const Component = (props) => {
    ...
    const [childCollections, setChildCollections] = useState([]);
        ...
        const [petCareCenterServices, setPetCareCenterServices] = useState([]);
    ...
    const OnSubmit = async () => {
        let rslt, data, prodImages, petCareCenterId, changes, numfields;
        const mapItems = MapItems;

        // Attach inline objects
        let petcarecenter = row['petcarecenter'];
        petCareCenterId = row['petcarecenter'].find((x) => x.type === 'keyid').value;
        ...
        changes = TrackChanges('petcarecenter');
        if (changes.length > 0) {
            numfields = Helper.GetAllNumberFields(petcarecenter);
            if (numfields.length > 0) Helper.UpdateNumberFields(petcarecenter, numfields);
        ...
        for (let i = 0; i < updateChild.length; i++) {
            let _data = updateChild[i];
                        let ServicesPetCareCenterMapId = null;
            if (_data.Deleted && !Helper.IsNullValue(_data.ServiceId)) {
                ServicesPetCareCenterMapId = petcarecenterServices.find(x => x.ServiceId === _data.ServiceId && x.PcId === petCareCenterId).Id;
            }
            rslt = await Support.AddOrUpdatePetCareCenterServices(ServicesPetCareCenterMapId, petCareCenterId, _data);
        ...
        // Add or Update PetCareCenter Logo
        changes = TrackChanges('petcarecenter');
        if (changes.length > 0 && changes.indexOf('Logo') > -1) {
    ...
    const fetchData = async () => {
        await Extract(id).then(rslt => {
            ...
            setDropDownOptions(options);
                        setPetCareCenterServices(mapitems)
